    ...
    if (ToastContext.pushNotification) {
      switch (type) {
        case 'success':
          ...
          ToastContext.pushNotification({
            msg: `hi ${mail}, you are successfully logged in.`,
            type: 'success',
          });
          break;
        case 'success-logout':
          ToastContext.pushNotification({
            msg: `hi , you are successfully logged out.`,
            type: 'success',
          });
          break;
        case 'failure':
        ...
        case 'tokenExpired':
          ToastContext.pushNotification({
            msg: `You got logged out login again.`,
            type: 'warning',
          });
          break;
        default:
...
class appContainer extends Component {
  ...
  popNotification = () => {
    ...
    this.notificationTimer = setTimeout(() => {
      this.setState((prevStat) => {
        let prestMsgs = [...prevStat.toastMessages.slice()];
        let openedNotifications = prestMsgs.filter(
          (msg) => new Date().getTime() - msg.id < 9500
        );
        return {
  ...
  render() {
    ...
    return (
      <ToastContext.Provider
        value={{ pushNotification: this.pushNotification }}
      >
        <AppWrapper toastMessagesLength={this.state.toastMessages.length} />
        <ToastMessage
          notifications={this.state.toastMessages}
          pushNotificatio={this.pushNotification}
